...
public class HashmapAndHeap {
    ...
    public static void getCommonElements(int[] a1, int[] a2) {
        ...
        for (int i = 0; i < a2.length; i++) {
            if (hm.containsKey(a2[i])) {
                ...
                hm.remove(a2[i]);     //to avoid duplicates
            }
        }
    ...
    public static void longestConsecutiveSS(int[] arr) {
        ...
        for (int i = 0; i < arr.length; i++) {  //for 10 
            if (!hm.containsKey(arr[i] - 1)) {  //if 9 doesn't exists
                hm.put(arr[i], true);           //put 10 -> true
            }
        }

        int lssl = 0;   //longest ss length
        int lss = 0;    //longest ss
        for (int val : arr) {
            if (hm.get(val)) {
                int ssl = 1;
                int ss = val;
