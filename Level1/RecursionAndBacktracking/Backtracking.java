...
public class Backtracking{
    public static void floodfill(int[][] maze, int sr, int sc, String asf,boolean[][] visited) {   //visited array why?
        ...
        visited[sr][sc]=true;   // why?     
        floodfill(maze, sr-1, sc, asf+"t",visited);
        floodfill(maze, sr, sc-1, asf+"l",visited);
        floodfill(maze, sr+1, sc, asf+"d",visited);
        floodfill(maze, sr, sc+1, asf+"r",visited);
         visited[sr][sc]=false;  //why?
    }
    ...
    public static boolean isQueenSafe(int[][] chess, int r, int c){
        for(int row=r-1; row>=0; row--){   //column 
            if(chess[row][c]==1) return false;
        }
        
        for(int row=r-1, col=c-1; row>=0 && col>=0; row--, col--){  //left diagonal
            if(chess[row][col]==1) return false;
        }
        
        for(int row=r-1, col=c+1; row>=0 && col<chess.length; row--, col++){  //right diagonal
            if(chess[row][col]==1) return false;
        }
